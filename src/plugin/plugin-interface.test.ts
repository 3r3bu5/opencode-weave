...
import { resetDisplayNames } from '../shared/agent-display-names';
...
describe('createPluginInterface', () => {
  ...
  it('event handler calls markSessionCreated on session.created event', async () => {
    ...
    await iface.event({
      event: event as Parameters<typeof iface.event>[0]['event'],
    });
  ...
  it('chat.message injects start-work context into existing text part in-place', async () => {
    ...
    const parts = [
      {
        type: 'text',
        text: '<session-context>Session ID: s1</session-context>',
      },
    ];
  ...
  it('chat.message substitutes $SESSION_ID and $TIMESTAMP in text parts before passing to startWork', async () => {
    ...
    const parts = [
      {
        type: 'text',
        text: '<session-context>Session ID: $SESSION_ID  Timestamp: $TIMESTAMP</session-context>',
      },
    ];
    const message: Record<string, unknown> = {};
    const output = { message: message as never, parts };
  ...
  it('event handler calls client.session.promptAsync when workContinuation returns a continuationPrompt', async () => {
    ...
    expect(promptAsyncCalls[0].body.parts[0].text).toBe(
  ...
  it('event handler does not throw when client is absent and continuationPrompt is set', async () => {
    ...
    const event = {
      type: 'session.idle',
      properties: { sessionID: 'sess-no-client' },
    };
    // Should not throw
    await expect(
      iface.event({
  ...
  describe('interrupt pausing (filesystem-based)', () => {
    ...
    it('suppresses work continuation after user interrupt and sets paused: true in state', async () => {
      ...
      const hooks = makeHooks({
        workContinuation: (sessionId: string) =>
          checkContinuation({ sessionId, directory: tempDir }),
      });
      ...
      const interruptEvent = {
        ...
        properties: { command: 'session.interrupt' },
      };
      await iface.event({
        event: interruptEvent as Parameters<typeof iface.event>[0]['event'],
      });
      ...
      const idleEvent = {
        type: 'session.idle',
        properties: { sessionID: 'sess-interrupted' },
      };
      await iface.event({
        event: idleEvent as Parameters<typeof iface.event>[0]['event'],
      });
    ...
    it('persistently suppresses continuation across multiple idle events (not one-shot)', async () => {
      ...
      await iface.event({
        event: idleEvent1 as Parameters<typeof iface.event>[0]['event'],
      });
      expect(promptAsyncCalls.length).toBe(0);
      ...
      await iface.event({
        event: idleEvent2 as Parameters<typeof iface.event>[0]['event'],
      });
      expect(promptAsyncCalls.length).toBe(0);
    });
    ...
    it('does not suppress continuation for non-interrupt TUI commands', async () => {
      ...
      await iface.event({
        event: compactEvent as Parameters<typeof iface.event>[0]['event'],
      });
      ...
      const idleEvent = {
        type: 'session.idle',
        properties: { sessionID: 'test-plan' },
      };
      await iface.event({
        event: idleEvent as Parameters<typeof iface.event>[0]['event'],
      });
  ...
  describe('auto-pause on user message during active plan', () => {
    ...
    it('auto-pauses work when a regular user message arrives during active plan', async () => {
      ...
      const idleEvent = {
        type: 'session.idle',
        properties: { sessionID: 'sess-user' },
      };
      await iface.event({
        event: idleEvent as Parameters<typeof iface.event>[0]['event'],
      });
    ...
    it('does NOT auto-pause when message contains continuation marker', async () => {
      const { CONTINUATION_MARKER } = await import(
        '../hooks/work-continuation'
      );
    ...
    it('does NOT auto-pause when message is a /start-work command', async () => {
      ...
      const output = {
        ...
        parts: [
          {
            type: 'text',
            text: '<session-context>Session ID: sess_test  Timestamp: 2026-01-01</session-context>',
          },
        ],
      };
    ...
    it('breaks the infinite continuation loop: user message → auto-pause → idle → no continuation', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        const idleEvent = {
          type: 'session.idle',
          properties: { sessionID: 'sess-loop' },
        };
        await iface.event({
          event: idleEvent as Parameters<typeof iface.event>[0]['event'],
        });
...
describe('context window monitoring', () => {
  ...
  it('chat.params captures model context limit into session token state', async () => {
    ...
    const input = {
      sessionID: 'sess-params',
      model: { limit: { context: 200_000 } },
    };
    await iface['chat.params'](
      input as Parameters<(typeof iface)['chat.params']>[0],
      {} as never,
    );
  ...
  it('event handler processes message.updated with assistant tokens and calls checkContextWindow', async () => {
    ...
    await iface['chat.params'](
      paramsInput as Parameters<(typeof iface)['chat.params']>[0],
      {} as never,
    );
...
describe('analytics: agent name and cost tracking', () => {
  ...
  beforeEach(() => {
    tempDir = join(
      tmpdir(),
      `weave-analytics-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
...
describe('command.execute.before handler', () => {
  ...
  beforeEach(() => {
    tempDir = join(
      tmpdir(),
      `weave-cmd-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
    mkdirSync(tempDir, { recursive: true });
  });
  ...
  it('injects report text for token-report command', async () => {
    ...
    const { appendSessionSummary } = await import(
      '../features/analytics/storage'
    );
    ...
    await iface['command.execute.before'](
      { command: 'token-report', sessionID: 's1', arguments: '' } as Parameters<
        (typeof iface)['command.execute.before']
      >[0],
    ...
    await iface['command.execute.before'](
      {
        ...
        sessionID: 's1',
        arguments: 'my-plan',
      } as Parameters<(typeof iface)['command.execute.before']>[0],
      output as Parameters<(typeof iface)['command.execute.before']>[1],
    );
...
describe('workflow integration in plugin-interface', () => {
  describe('auto-pause guard recognizes WORKFLOW_CONTINUATION_MARKER', () => {
    ...
    it('does NOT auto-pause when message contains WORKFLOW_CONTINUATION_MARKER', async () => {
      const { WORKFLOW_CONTINUATION_MARKER } = await import(
        '../features/workflow/hook'
      );
  ...
  describe('message.part.updated text tracking', () => {
    it('tracks assistant message text from message.part.updated events', async () => {
      ...
      const hooks = makeHooks({
        workflowContinuation: (
          ...
          if (
            ...
            lastAssistantMessage.includes('<!-- workflow:step-complete -->')
          ) {
            return {
      ...
      await iface.event({
        event: partEvent as Parameters<typeof iface.event>[0]['event'],
      });
  ...
  describe('auto-pause suppression during active workflow', () => {
    ...
    function setupRunningWorkflowInstance(dir: string) {
      const {
        ...
        setActiveInstance,
      } = require('../features/workflow/storage');
      const {
        WORKFLOWS_STATE_DIR,
        WORKFLOWS_DIR_PROJECT,
      } = require('../features/workflow/constants');
  ...
  describe('todo finalization safety net', () => {
    ...
    it('injects finalize prompt when session.idle fires with in_progress todos and no continuation', async () => {
      ...
      await iface.event({
        event: evt as Parameters<typeof iface.event>[0]['event'],
      });

      expect(promptAsyncCalls.length).toBe(1);
      expect(promptAsyncCalls[0].path.id).toBe('sess-finalize-1');
      expect(promptAsyncCalls[0].body.parts[0].text).toContain(
    ...
    it('does not inject finalize prompt when workContinuation fires', async () => {
      ...
      expect(promptAsyncCalls[0].body.parts[0].text).not.toContain(
        '<!-- weave:finalize-todos -->',
      );
    });
  ...
  describe('tool.definition handler', () => {
    it('calls todoDescriptionOverride when toolID is todowrite and hook is enabled', async () => {
      ...
      await iface['tool.definition'](
        { toolID: 'todowrite' } as Parameters<
          (typeof iface)['tool.definition']
        >[0],
    ...
    it('does not mutate description for non-todowrite tools when real hook is wired', async () => {
      const { applyTodoDescriptionOverride } = await import(
        '../hooks/todo-description-override'
      );
      const iface = createPluginInterface({
        pluginConfig: baseConfig,
        hooks: makeHooks({
          todoDescriptionOverride: applyTodoDescriptionOverride,
        }),
        tools: emptyTools,
        configHandler: makeMockConfigHandler(),
        agents: {},
  ...
  describe('experimental.session.compacting handler', () => {
    it('calls compactionPreserver.capture when compactionTodoPreserverEnabled is true', async () => {
      ...
      await iface['experimental.session.compacting'](
        { sessionID: 'ses-compact-1' } as Parameters<
          (typeof iface)['experimental.session.compacting']
        >[0],
    ...
    it('is no-op when compactionTodoPreserverEnabled is false (no client instantiated)', async () => {
      ...
      await expect(
        iface['experimental.session.compacting'](
          { sessionID: 'ses-compact-disabled' } as Parameters<
            (typeof iface)['experimental.session.compacting']
          >[0],
          {} as Parameters<
            (typeof iface)['experimental.session.compacting']
